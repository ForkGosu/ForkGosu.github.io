...
function inputChange() {
    ...
    if (val !== '') {
        ...
        if (val === 'in:') {
            $('#no-item-tips').show().html('글로벌 키워드 검색 진행 중 입니다, 키워드를 입력해주세요');
        } else if (!val.startsWith('in:') && $(".nav-right nav a:visible").length === 0) {
            $('#no-item-tips').show().html('이곳은 아닙니다 <span>' + activeTitle + '</span> 카테고리에서 발견됨'+ searchType + containType + ' <span>' + val.replace(/^[@|#]/g,'') + '</span> 조항');
        }
    } else {
...
function syncOutline(_this) {
    try{
        ...
        console.error('TOC 위치를 동기화하지 못했습니다!', e)
    }
}
...
function bind() {
    ...
    if ($('#theme_highlight_on').val() === 'true') {
        $('pre code').each(function (i, block) {
            ...
            // 添加复制功能
            $(this).after('<div class="code-embed"><span class="code-embed-type">'+ (codeClass.indexOf('hljs') === -1 ? codeClass : codeClass.indexOf('hljs') === 0 ? '' : codeClass.replace(/[\s]?hljs/g, ''))+'</span>'+(hasCopy ? '<span class="code-embed-copy" onclick="copyCode(this)">코드 복사</span>' : '')+'</div>')
            // 渲染样式
